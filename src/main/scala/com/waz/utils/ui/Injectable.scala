...
trait Injector { self =>
  ...
  private[ui] val head = self
  private[ui] val tail = self

  private[ui] var parent = Option.empty[Injector]

  def ::(inj: Injector) = new Injector {
    inj.tail.parent = Some(self.head)

    override private[ui] val head: Injector = inj.head
    override private[ui] val tail: Injector = self.tail
...
class Module extends Injector with Injectable {
  ...
  private[ui] def internal[T](m: reflect.Manifest[T]) = bindings.get(m).asInstanceOf[Option[() => T]]
